...
export async function PATCH(req, { params }) {
  ...
  if (requestedStatus) {
    ...
    if (requestedStatus === "completed" && !body.completedAt)
  ...
  if (typeof body.estimatedHours === "number")
    update.estimatedHours = body.estimatedHours;
  if (typeof body.actualHours === "number")
    update.actualHours = body.actualHours;
  ...
  if (body.parentTask !== undefined && body.parentTask !== null) {
    ...
    if (await isDescendant(id, body.parentTask)) {
      return NextResponse.json(
        {
          error: true,
          message: "Cannot set a descendant as parent (would create cycle)",
        },
        { status: 400 }
      );
    }
  ...
  if (Array.isArray(body.dependencies) && body.dependencies.length > 0) {
    async function hasDependencyCycle(taskId, depIds, visited = new Set()) {
      ...
      for (const depId of depIds) {
        ...
        if (
          ...
          if (await hasDependencyCycle(taskId, depTask.dependencies, visited))
            return true;
        }
      }
      return false;
    ...
    if (await hasDependencyCycle(id, body.dependencies)) {
      return NextResponse.json(
        { error: true, message: "Circular dependency detected" },
        { status: 400 }
      );
    }
  ...
  if (
    requestedStatus &&
    ["in_progress", "review", "completed"].includes(requestedStatus)
  ) {
    const current = await Task.findById(id)
      .select("dependencies status")
      .lean();
    if (!current)
      return NextResponse.json(
    ...
    if (deps.length > 0) {
      ...
      if (blockers > 0) {
        return NextResponse.json(
          {
            error: true,
            message: `Task has ${blockers} incomplete dependency(ies)`,
          },
          { status: 400 }
        );
      }
  ...
  const before = await Task.findById(id)
    .select(
      "title description milestone assignee assignees status priority taskType startDate dueDate estimatedHours actualHours progress tags dependencies checklist attachments recurring parentTask boardOrder project"
    )
    .lean();
  if (!before)
    return NextResponse.json(
  ...
  try {
    if (typeof body.progress !== "number") {
      ...
      const [subTotal, subCompleted] = await (async () => {
        const subs = await Task.find({ parentTask: id })
          .select("status")
          .lean();
      ...
      if (parts.length > 0)
        computed = Math.round(parts.reduce((a, b) => a + b, 0) / parts.length);
  ...
  try {
    if (requestedStatus === "completed") {
      const dependents = await Task.find({
        ...
        .select("_id dependencies status")
        .lean();
  ...
  // Workflow trigger: Notify HR/manager when task is completed
  try {
    if (requestedStatus === "completed") {
      notifyTaskCompletion(id).catch((err) => {
  ...
  try {
    if (requestedStatus === "completed") {
      triggerWebhooks("task.completed", {
        taskId: updated._id.toString(),
        title: updated.title,
        status: updated.status,
        project: updated.project?.toString(),
        assignee: updated.assignee?.toString(),
        completedAt: updated.completedAt,
      }).catch((err) => {
        console.error("Failed to trigger task.completed webhooks:", err);
      });
    } else {
      triggerWebhooks("task.updated", {
        taskId: updated._id.toString(),
        title: updated.title,
        status: updated.status,
        project: updated.project?.toString(),
        ...
        console.error("Failed to trigger task.updated webhooks:", err);
      });
    }
  } catch (e) {}
...
export async function DELETE(_req, { params }) {
  ...
  try {
    triggerWebhooks("task.deleted", {
      taskId: task._id.toString(),
      title: task.title,
      project: task.project?.toString(),
    }).catch((err) => {
      console.error("Failed to trigger task.deleted webhooks:", err);
    });
  } catch (e) {}
  ...
  return NextResponse.json(
    { error: false, message: "Task deleted" },
    { status: 200 }
  );
}
