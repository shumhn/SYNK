...
/**
 * GET /api/analytics/hr/employee/:id/trends?period=daily|weekly|monthly&days=30
 * Returns productivity trends scoped to a single employee (assignee)
 */
export async function GET(req, { params }) {
  ...
  try {
    ...
    // Completed by period (for this assignee)
    const completedTrend = await Task.aggregate([
      { $match: { status: "completed", assignee: new mongoose.Types.ObjectId(id), completedAt: { $gte: startDate, $lte: now } } },
      { $group: { _id: groupBy, count: { $sum: 1 } } },
      { $sort: { _id: 1 } },
    ]);

    // Created by period (tasks created and assigned to this user)
    const createdTrend = await Task.aggregate([
      { $match: { assignee: new mongoose.Types.ObjectId(id), createdAt: { $gte: startDate, $lte: now } } },
      { $group: { _id: { $dateToString: { format: dateFormat, date: "$createdAt" } }, count: { $sum: 1 } } },
      { $sort: { _id: 1 } },
    ]);

    // In-progress snapshot approx
    const inProgressTrend = await Task.aggregate([
      { $match: { status: "in_progress", assignee: new mongoose.Types.ObjectId(id), createdAt: { $lte: now } } },
      { $group: { _id: { $dateToString: { format: dateFormat, date: "$createdAt" } }, count: { $sum: 1 } } },
      { $sort: { _id: 1 } },
    ]);
    ...
    console.error("Error fetching employee productivity trends:", e);
