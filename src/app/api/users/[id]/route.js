...
export async function PATCH(req, { params }) {
  ...
  if (update.email || update.username) {
    ...
    if (conflict) return NextResponse.json({ error: true, message: "Email or username already in use" }, { status: 400 });
  }
...
export async function DELETE(_req, { params }) {
  ...
  if (!res) return NextResponse.json({ error: true, message: "Not found" }, { status: 404 });
  return NextResponse.json({ error: false, message: "User deleted" }, { status: 200 });
}
